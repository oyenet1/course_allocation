...
<?php $component = App\View\Components\Form::resolve(['title' => 'allocation','update' => $update] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? (array) $attributes->getIterator() : [])); ?>
...
<?php $component->withAttributes([]); ?>
      ...
      <div class="w-full h-12 space-y-1">
        <select wire:model.defer="user_id" class='w-full h-full pl-4 space-y-1 font-medium text-gray-500 placeholder-gray-500 capitalize bg-gray-100 border-0 rounded peer tt focus:border-2 focus:border-primary focus:bg-white focus:outline-none' id="">
          <option class="text-sm">Select Lecturer*</option>
          <?php $__currentLoopData = \App\Models\User::select(['id', 'first_name', 'title', 'last_name'])->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <?php if($option->id === 1): ?>
          <?php continue; ?>
          <?php endif; ?>
          <option class="py-2 capitalize" value="<?php echo e($option->id); ?>"><?php echo e($option->title . ' '.$option->first_name . ' '. $option->last_name); ?></option>
...
unset($__errorArgs, $__bag); ?>
      ...
      <div class="w-full h-12 space-y-1">
        <select wire:model.defer="course_id" class='w-full h-full pl-4 space-y-1 font-medium text-gray-500 placeholder-gray-500 capitalize bg-gray-100 border-0 rounded peer tt focus:border-2 focus:border-primary focus:bg-white focus:outline-none' id="">
          <option class="text-sm">Select Course*</option>
          <?php $__currentLoopData = \App\Models\Course::select(['id', 'title', 'code'])->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <option class="py-2 capitalize" value="<?php echo e($option->id); ?>"><?php echo e($option->code . ' ('. $option->title. ')'); ?></option>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </select>
...
unset($__errorArgs, $__bag); ?>
      ...
      <div class="w-full h-12 space-y-1">
        <select wire:model.defer="semester_id" class='w-full h-full pl-4 space-y-1 font-medium text-gray-500 placeholder-gray-500 capitalize bg-gray-100 border-0 rounded peer tt focus:border-2 focus:border-primary focus:bg-white focus:outline-none' id="">
          <option class="text-sm">Select Semester*</option>
          <?php $__currentLoopData = \App\Models\Semester::select(['id', 'name'])->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<div class="flex items-center justify-between ">
  <div class="flex items-center space-x-6">
    <h1 class="text-3xl font-semibold capitalize text-primary">Allocation</h1>
    <?php if($checked): ?>
...
<?php $component = App\View\Components\Button\Add::resolve(['name' => 'course','precede' => 'allocate'] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? (array) $attributes->getIterator() : [])); ?>
...
<div class="w-full px-4 pb-4 overflow-x-auto bg-white rounded-lg shadow-sm">
  <?php if(\App\Models\CourseAllocation::count() > 1): ?>
  <table class="w-full space-y-2 overflow-x-auto whitespace-nowrap">
    <thead class="w-full pb-4 text-xl border-b">
      <tr class="font-medium">
        ...
        <th class="p-2 text-xl font-medium text-left">Course</th>
        <th class="p-2 text-xl font-medium text-left">Lecturer Assigned</th>
        <th class="p-2 text-xl font-medium text-left">Level</th>
        <th class="p-2 text-xl font-medium text-left">Semester</th>
        <th class="p-2"></th>
      </tr>
    </thead>
    <tbody class="w-full overflow-x-auto break-normal">
      ...
      <?php if($allocation->id === 1): ?>
      <?php continue; ?>
      <?php endif; ?>
      <tr class="">
        <td class="p-2 whitespace-nowrap">
          <input type="checkbox" wire:model="checked" id="" value="<?php echo e($allocation->id); ?>" class="block rounded whitespace-nowrap text-primary focus:outline-none focus:ring-primary">
        </td>
        <td class="p-2 whitespace-nowrap">
          <p class=""><?php echo e($allocation->course->code); ?></p>
        </td>
        <td class="p-2 whitespace-nowrap">
          <p class=""><?php echo e($allocation->user->title . ' ' .$allocation->user->first_name . ' '. $allocation->user->last_name); ?></p>
        </td>
        <td class="p-2 whitespace-nowrap">
          <?php echo e($allocation->course->level->name); ?>

        </td>
        <td class="p-2 whitespace-nowrap capitalize">
          <?php echo e($allocation->semester->name); ?>

        </td>
        <td class="p-2 whitespace-nowrap">
          <div class="flex space-x-2 item-center">
            
            <span wire:click="edit(<?php echo e($allocation->id); ?>)" class="w-8 h-8 p-2 text-blue-600 border border-blue-600 rounded-md cursor-pointer tt hover:-translate-y-1">
            ...
            <span wire:click="confirmDelete(<?php echo e($allocation->id); ?>)" class="w-8 h-8 p-2 text-red-600 border border-red-600 rounded-md cursor-pointer tt hover:-translate-y-1">
...
<?php /**PATH /Users/user/Documents/projects/course_allocation/resources/views/livewire/allocation.blade.php ENDPATH**/ ?>